...
controller.login = function (request, response) {
	...
	User.find({ email: request.body.email, pass: request.body.password }, function (err, data) {
		if (err) {
			//throw new Error();
			return response.json({ message: err.Errors });
		}
		if (data.length > 0) {
...
controller.update = function (request, response) {
	...
	if (request.body.email) {
		return response.status(400).json({ message: 'Mail alanı eksik' });
	}
	if (request.body.password) {
		return response.status(400).json({ message: 'Şifre alanı eksik!' });
	}
